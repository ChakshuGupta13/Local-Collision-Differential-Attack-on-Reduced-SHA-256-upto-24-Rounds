...
uniform_int_distribution<mt19937::result_type> udist(0, 7);
...
int main()
{
    ...
    string user_given_password;
    cin >> user_given_password;

    auto epoch = now.time_since_epoch();
    mt19937::result_type const seedval = duration_cast<nanoseconds>(epoch).count(); // get this from somewhere
    random_number_generator.seed(seedval);

    mt19937::result_type random_number = udist(random_number_generator);

    stringstream random_number_in_hex;
    random_number_in_hex << setfill('0') << setw(2) << hex << uppercase << random_number;
    string nonce = random_number_in_hex.str();

    cout << "Hashed Password: " << SHA512(user_given_password + nonce) << "\n";
}